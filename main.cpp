...
int main()
{
    ...
    while (!exit)
    {
        ...
        window.setVerticalSyncEnabled(true);//óïðàâëåíèå êàäðàìè
...
///============================///
        ...
        glDepthMask(GL_TRUE);
        glClearDepth(1.f);
        glDisable(GL_LIGHTING);// îòêëþ÷åíèå îñâåùåíèÿ
        //glViewport(0, 0, window.getSize().x, window.getSize().y);//îáëàñòü íà ýêðàíå , ãäå âûâîäèòñÿ èçîáðàæåíèå
            ...
            while (window.pollEvent(event))
            {
                ...
                if ((event.type == Event::KeyPressed) )
                {
                    switch(event.key.code)
                    {
                        ...
                        case Keyboard::F1:
                            {
                                system("notepad resources/ñïðàâêà.txt");
                                break;
                            }
                    }
...
/**
            ...
            ImGui::Begin("Nastroiki:"); // ñîçäà¸ì îêíî
            ImGui::NewFrame;
